...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetchCurrentReadings(); // Fetch latest readings on component mount
    fetch24hSensorData();   // Fetch last 24-hour sensor data
    fetchForecastData();    // Fetch forecast data

    const intervalCurrent = setInterval(fetchCurrentReadings, 6000);  // Every minute 60sec
    
    const interval24h = setInterval(fetch24hSensorData, 6000);       // Every minute
    const intervalForecast = setInterval(fetchForecastData, 60000);  // Every 10 minutes
  ...
  return (
    ...
    <div className={`${styles.reading} ${styles.animated}`}>
        <img
